...
- `body_text`(每篇文章,英文,`fetch_pmc.py` 抓 JATS `<body>` 段落,过滤 <40 字符短段,限 8000 字符防 docx 爆)
- `figures`(JATS `<fig>` 解析:`label` + `caption` + `url`)

`render.py --mode single` 把这俩字段渲染成两个新章节:
- `## 📖 原文主体 / Full Text Body` — body 段落,英文原文(限 `max_body_chars=4000` 默认)
- `## 🖼️ 图片 / Figures(3/6)` — 前 `max_figures=3` 张图 inline 嵌入(飞书 `<img href="..."/>` 自动下载), caption 跟在图后;超出部分提示"详见 PMC 全文链接"
